$(function(){
	$(".board-bottom .paging-nav").bind('click', function(){
		...
		var hash = 'board?url=' + boardVar.tag + '&page=' + page;
		if(boardVar.filter != '0'){
			hash += '&filter=' + boardVar.filter;
		}
		location.hash = hash;
	});
	$(".board-top .sort").bind('change', function(){
		var filter = $(this).val();
		var hash = 'board?url=' + boardVar.tag + '&filter=' + filter;
		location.hash = hash;
	});
...
function submit(){
	...
	$.ajax({
		...
		type : "post",
		data :{
			req_type : "askboard_sticky_register",
