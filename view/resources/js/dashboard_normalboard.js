...
$(function(){
	$(".board-bottom .paging-nav").bind('click', function(){
		...
		hash = 'board?url=' + boardVar.tag + '&page=' + page;
		location.hash = hash;
	});
	...
	tinymce.init({
	    ...
	    toolbar: "undo redo | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image | fontsizeselect forecolor | uploadpicture | media",
	...
	$(".board-wrapper .board-top .compose-button").bind('click', function(){
		$(".board-wrapper").fadeOut(150, function(){
	...
	$(".board-center .table-body .item-wrapper").bind('click', function(){
		...
		location.hash = 'board?url=' + boardVar.tag + '&seq=' + seq;
	});

	$(".board-compose-wrapper .input-line .sticky-input").switchButton({
		checked: boardVar.sticky,
		on_label: stringTable.stickyPost,
		off_label: stringTable.normalPost,
		width: 60,
		height: 30,
		button_width: 30,
		on_callback: function(){
			boardVar.sticky = true;
		},
